        ...
        <table className="w-full text-right text-sm">
          <thead className="bg-slate-50 dark:bg-slate-800/60 text-xs font-bold text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
            <tr>
              ...
              <th className="py-4 px-5">المستودع المصدر</th>
              <th className="py-4 px-5">المادة الموزعة</th>
              <th className="py-4 px-5">الكمية المسلمة</th>
              <th className="py-4 px-5">تاريخ ووقت التسليم</th>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-200">
            {isLoading ? (
              ...
              filteredDistributions.map((dist) => {
                ...
                return (
                  <tr key={dist.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                    {/* Family */}
                    ...
                    {/* Depot */}
                    <td className="py-4 px-5 text-xs font-bold text-slate-800 dark:text-slate-200">
                    ...
                    {/* Quantity */}
                    <td className="py-4 px-5 font-mono font-bold text-base text-slate-900 dark:text-white">
                    ...
                    <td className="py-4 px-5 text-xs text-slate-500 dark:text-slate-400 font-mono">
                      {dist.distributedAt ? new Date(dist.distributedAt).toLocaleString('ar-DZ') : '—'}
                    </td>

                    {/* Notes */}
                    <td className="py-4 px-5 text-xs text-slate-500 dark:text-slate-400 max-w-xs">
                      {dist.notes ? (
                        <span className="block truncate" title={dist.notes}>
                          {dist.notes}
                        </span>
                      ) : (
