      ...
      <div className="overflow-x-auto rounded-3xl border border-slate-200/90 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xs">
        <table className="w-full text-right text-sm">
          <thead className="bg-slate-50 dark:bg-slate-800/60 text-xs font-bold text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
            <tr>
              <th className="py-4 px-5">رب الأسرة / الكود</th>
              <th className="py-4 px-5">عدد الأفراد</th>
              <th className="py-4 px-5">الموقع ومركز الإيواء</th>
              <th className="py-4 px-5">رقم الهاتف</th>
              <th className="py-4 px-5">الحالة</th>
              <th className="py-4 px-5">الإعانات المستلمة</th>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-200">
            {isLoading ? (
              ...
              filteredFamilies.map((fam) => {
                ...
                return (
                  <tr key={fam.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                    {/* Head of Family */}
                    ...
                    <td className="py-4 px-5">
                      ...
                      <span className="text-xs text-slate-400 mr-1">أفراد</span>
                    </td>

                    {/* Shelter / Location */}
                    <td className="py-4 px-5 text-xs">
                      <div className="space-y-0.5">
                        <p className="font-bold text-slate-800 dark:text-slate-200">{fam.location}</p>
                        <p className="text-slate-400">بلدية {fam.commune} • ولاية {fam.wilaya}</p>
                      </div>
                    </td>

                    {/* Phone */}
                    <td className="py-4 px-5 font-mono text-xs text-slate-600 dark:text-slate-300" dir="ltr">
                      {fam.phone || '—'}
                    </td>
                    ...
                    {/* Distributions count */}
                    <td className="py-4 px-5 font-mono text-center">
                      <span className="inline-flex items-center justify-center h-7 w-7 rounded-full bg-slate-100 dark:bg-slate-800 text-xs font-bold">
                        {fam.totalDistributionsReceived || 0}
                      </span>
                    </td>
                    ...
                    <td className="py-4 px-5 text-xs text-slate-500 dark:text-slate-400 max-w-xs">
                      {fam.notes ? (
                        <span className="block truncate" title={String(fam.notes)}>
                          {String(fam.notes)}
                        </span>
                      ) : (
